...
def getNeuralOutput(individual,item):
    ...
    for i in range(0, len(individual) / len(item) - 1): #iterate through layers
        ...
        for l in range(0, len(item)): #iterate through each node
            nextLayer.append(getSigmoidZ(oldLayer,individual[index:(index+len(item))]));
            index+=1;
        oldLayer = nextLayer;
...
numLayers = 2; #more layers -> better complexity -> more time to train
numBiases = len(bias); #a bit more complexity but not as much as more layers, however doesn't take as much time
